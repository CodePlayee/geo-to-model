...
- **行政区索引**：`scripts/gen-regions.mjs` 爬取 DataV（国→省→市→区县）生成离线索引 `src/regions-index.json`（adcode/名称/拼音/中心点，约 0.4 MB），供搜索与级联下拉使用；区县边界几何在选中时按 adcode 按需拉取，不打包。重新生成：`npm run tb:regions`。
